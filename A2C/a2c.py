    ...
    def learn(self, total_timesteps, callback=None, seed=None, log_interval=100, tb_log_name="A2C",
        ...
        with SetVerbosity(self.verbose), TensorboardWriter(self.graph, self.tensorboard_log, tb_log_name, new_tb_log) \
            ...
            for update in range(1, total_timesteps // self.n_batch + 1):
                ...
                obs, states, rewards, masks, actions, values, ep_infos, true_reward, dones = runner.run()
                ...
                if self.verbose >= 1 and (update % log_interval == 0 or update == 1):
                    ...
                    logger.dump_tabular()
                self.reward += sum(true_reward)
                if writer is not None:
                    if self.early_stop or dones.any():
                        print('Reward: {}'.format(self.reward))
                        summary = tf.Summary(value=[tf.Summary.Value(tag="episode_reward", simple_value=self.reward)])
...
class A2CRunner(AbstractEnvRunner):
    ...
    def run(self):
        ...
        for i in range(self.n_steps):
            ...
            # for _ in range(self.model.skip_frame):
            ...
            # # early stop
            # if self.dones:
            #     self.model.neg_reward_counter = 0
            # if (rewards < 0):
            #     self.model.neg_reward_counter += 1
            #     if (self.model.neg_reward_counter > 25):
            #         self.model.neg_reward_counter = 0
            #         self.model.early_stop = True
            #         # print(sum(rewards > 0 for rewards in mb_rewards))
            #         mb_rewards[-1] = [-20.0] # /(sum(rewards > 0 for rewards in mb_rewards)*0.5+1)]# [-20.0/(self.model.num_timesteps/1000)]
            #     # print(self.model.neg_reward_counter)
            #     # print(mb_rewards)
            # else:
            #     self.model.neg_reward_counter = 0
        mb_dones.append(self.dones)
        ...
        return mb_obs, mb_states, mb_rewards, mb_masks, mb_actions, mb_values, ep_infos, true_rewards, mb_dones
