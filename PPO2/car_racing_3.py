...
class CarRacing3(gym.Env, EzPickle):
    ...
    def _set_config(self, grayscale=True, vae_input=False, vae_path='/fzi/ids/habenich/no_backup/env_v/env_v4/lib/python3.5/site-packages/gym/envs/box2d/vae_gray.torch', skip_frames=3, num_stack=4, continuous=True, render_on_learn=False):
        ...
        if (num_stack>1):
            assert self.grayscale, "Frames can only be stacked when using grayscale"

        assert self.skip_frames > 0, "Skip frames must be 1 for no skipping or more for skipping frames"
        ...
        if self.grayscale:
            ...
            if self.num_stack > 1:
                self.observation_space = spaces.Box(low=0, high=255, shape=(80, 80, self.num_stack), dtype=np.uint8)
        else:
            self.observation_space = spaces.Box(low=0, high=255, shape=(80, 80, 3), dtype=np.uint8)
        if self.vae_input:
            self.observation_space = spaces.Box(low=-2, high=2, shape=(32,), dtype=np.float32)


    def preprocess_observation(self, observation, is_start=False):
        s = observation[0:80, 7:87]  # crop bottom bar and 8 pixels left/right for quadratic image
        ...
        if self.vae_input:
            ...
            # if self.steps%10==0:
            #     recon_s, _, _ = self.vae_model(s)
            #     compare_s = torch.cat([s, recon_s])
            #     save_image(compare_s.data.cpu(), 'sample_image_frame{}.png'.format(self.steps))
            #     image = Image('sample_image.png', width=700, unconfined=True)
            self.state = z.detach().numpy()[0]
            self.observation_space = spaces.Box(low=-2, high=2, shape=self.state.shape, dtype=np.float32)
            return self.state
            ...
            if self.grayscale:
                ...
                if self.num_stack > 1:
                    ...
                    if is_start or self.state is None:
                        self.state = np.repeat(s, self.num_stack, axis=3)
                    else:
                        self.state = np.append(s, self.state[:, :, :, :self.num_stack-1], axis=3)
    ...
    def reset(self):
        ...
        return self.step(None, is_start=True)[0]
    ...
    def step(self, action, is_start=False):
        ...
        for _ in range(self.skip_frames):
            ...
            if self.render_on_learn:
                self.render("human")
                
            s = self.render("state_pixels")
            step_reward = 0
            done = False
            ...
            accum_reward += step_reward


        return self.preprocess_observation(s, is_start), accum_reward, done, {}
