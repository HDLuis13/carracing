...
class RacingGym:
    ...
    def preprocess(self, obs, is_start=False):
        if self.vae:
            torchvision.transforms.Compose([torchvision.transforms.Resize
            ])
            s = obs[8:72, 14:78, :]
            s = s.reshape(1, s.shape[2], s.shape[0], s.shape[1])
            device = torch.device('cpu')
            vae = VAE(image_channels=3).to(device)
            vae.load_state_dict(torch.load('./PPO2/vae.torch', map_location='cpu'))
            z = vae.representation(torch.from_numpy(s).float())
            self.state = z.detach().numpy()[0]
            self.observation_space = spaces.Box(low=-1, high=1, shape=self.state.shape, dtype=np.float32)
            # s_g
