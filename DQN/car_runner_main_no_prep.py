...
Optionally save checkp
oints to checkpoint_path (set checkpoint_path=None to not save anything)
Experience history is never saved

Training and playing can be early stopped by giving input (pressing enter in console)
"""

from dqn.agent_no_prep import CarRacingDQN2
import os
...
checkpoint_path = "data/checkpoint06"
...
dqn_agent = CarRacingDQN2(env=env, **model_config)
dqn_agent.build_graph()
sess = tf.InteractiveSession()
dqn_agent.session = sess

saver = tf.train.Saver(max_to_keep=100)
train_writer = tf.summary.FileWriter('./logs/{}/'.format(checkpoint_path), sess.graph)
...
def one_episode():
    ...
    summary = tf.Summary(value=[tf.Summary.Value(tag="episode_reward", simple_value=reward)])
    train_writer.add_summary(summary, dqn_agent.episode_counter)
    summary = tf.Summary(value=[tf.Summary.Value(tag="episode_reward per timestep", simple_value=reward)])
    train_writer.add_summary(summary, dqn_agent.global_counter)
